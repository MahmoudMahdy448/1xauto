...
> **Version**: 1.8 · **Last Updated**: 2026-08-03 · **Status**: Authoritative
>
...
| P7 — Cloud VM as primary runtime | In Progress | — | C7 | Provisioning `scripts/vm-setup.sh` (Node 22, swap 2 GiB, clone, `npm install`, `playwright install --with-deps chromium`, 60-min cron with `LOW_MEMORY=true`) ready. Oracle path blocked (card verification) → **Azure** target: free-tier `Standard_B2ats_v2` (1 GiB) in South Africa North, 750 h/mo shared across shards. Sharding tooling done (`END_INDEX` in `lib/state.js` + spec; per-shard `STATE_FILE`; `vm-setup.sh` accepts `SHARD_START`/`SHARD_END`/`SHARD_STATE`). 69/69 unit tests green; dry-run slice verified (`START_INDEX=1 END_INDEX=5`). C7: provision VM(s), upload secrets, cron fires + Telegram notifies |
