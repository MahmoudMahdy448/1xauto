...
> **Version**: 1.10 · **Last Updated**: 2026-08-03 · **Status**: Authoritative
>
...
| P7 — Cloud VM as primary runtime | In Progress | — | C7 | Two-phase Azure plan (2026-08-03): **Phase A** — $200 credit month on a single `B2as v2` 8 GiB (deployed: `betting1xbet` in UAE North, 20.174.2.211, ~$66/mo), full batch; **Phase B** — deallocate, free-tier `B2ats_v2` (1 GiB) shards with `LOW_MEMORY=true` + 2 GiB swap + `END_INDEX` sharding. VM validated end-to-end (dry-run 1 passed, 276 accounts parsed). Runtime now **two parallel systemd loop services** (`scripts/register-shards.sh`, shards 1–149 / 150–276) running `run-loop.js` with per-shard `STATE_FILE`/`SCREENSHOTS_DIR`/`RUN_SUMMARY_FILE`/`EXCEL_FILE` + 60-min cooldown; notify posts **summary + deduped screenshots** to Telegram (`sendTelegramPhoto`). 72/72 unit tests green. C7: set `TELEGRAM_BOT_TOKEN`/`TELEGRAM_CHAT_ID`, `SHARDS="1:149,150:276" sudo bash scripts/register-shards.sh`, verify both services + Telegram images |
