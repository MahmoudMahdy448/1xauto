...
| **+ (deploy)** | `scripts/vm-setup.sh` | 106 | Ubuntu VM provisioning (P7): apt prereqs, RAM auto-detect (<2 GiB → swap + `LOW_MEMORY=true`), Node 22 via NodeSource, clone repo to `/opt/1xauto`, `npm install`, `playwright install --with-deps chromium`, 60-min cron for `scheduled-run.mjs` (with `SHARD_*` + `LOW_MEMORY` env), secret-upload reminder |
