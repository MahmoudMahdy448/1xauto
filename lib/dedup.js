...
export async function releaseNumbers({ filePath, lockPath, numbers }) {
  return withSeenNumbersLock(lockPath, () => {
    const seen = readSeenNumbers(filePath);
    const toRemove = new Set(numbers);
    const kept = [...seen].filter((n) => !toRemove.has(n));

    if (kept.length !== seen.size) {
      writeSeenNumbers(filePath, kept);
    }

    return numbers.length - kept.length;
  });
}
