...
async function renderApp(def) {
  ...
  const logTexts = await Promise.all(
    def.shards.map(async (s) => {
      ...
      if (s.remote) {
        try {
          return readFileSync(path.join(def.dir, `loop-log-${s.service.replace('1xauto-shard-', '')}.txt`), 'utf8');
        } catch {
          return 'n/a';
        }
      }
  ...
  def.shards.forEach((s, i) => {
    ...
    let stateLabel;
    if (s.remote) {
      stateLabel = loop?.state ?? 'no status';
    ...
    lines.push(`${s.service}: ${stateLabel}`);

    if (isRunning && progress?.current) {
      const within = withinShard(progress.current, startIndex);
      lines.push(`  currently processing acc ${within}${shardTotal ? ` out of ${shardTotal}` : ''}`);
    } else if (loop?.state === 'cooldown' && loop.nextRunAt) {
      lines.push(`  starting at ${shortTime(loop.nextRunAt)}`);
    }

    if (st && st.lastProcessedIndex != null) {
      lines.push(`  last processed: ${withinShard(st.lastProcessedIndex, startIndex)}`);
    }
    if (lastError) {
      const reason = (lastError.reason || '').slice(0, 80);
      lines.push(`  last error: ${lastError.email}${reason ? ` — ${reason}` : ''}`);
    }
    if (sum) {
      lines.push(`  last run: ${sum.succeeded} success, ${sum.failed} errors`);
    }
  });
