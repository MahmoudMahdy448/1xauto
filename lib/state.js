...
export function resolveEndIndex(explicitEndIndex, totalAccounts) {
  if (explicitEndIndex === undefined || explicitEndIndex === null) {
    return totalAccounts;
  }

  if (explicitEndIndex < 1) {
    throw new Error(`Invalid END_INDEX: "${explicitEndIndex}". It must be a positive integer.`);
  }

  return Math.min(explicitEndIndex, totalAccounts);
}
